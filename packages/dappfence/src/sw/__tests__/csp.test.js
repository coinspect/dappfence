...
import { normalizeManifestData } from '../manifest/manifest-loader.js';
...
// buildCspHeader takes (fileKey, response, manifest, nonce) and returns a Headers
// instance. `csp()` calls the helper with an empty-body response and returns the
// composed CSP header value so the existing string-oriented assertions read cleanly.
const emptyResponse = () => new Response(null, { headers: new Headers() });
const csp = (manifest, fileKey = '/', nonce = NONCE) => {
    const normalized = normalizeManifestData({ files: {}, ...(manifest ?? {}) });
    return buildCspHeader(fileKey, emptyResponse(), normalized, nonce).get(
...
describe('buildCspHeader — origin CSP stripping', () => {
    it('strips origin Content-Security-Policy and replaces with the built policy', () => {
        ...
        const headers = buildCspHeader('/', response, {}, NONCE);
